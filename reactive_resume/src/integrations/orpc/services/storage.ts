...
class ResilientStorageService implements StorageService {
  ...
  async list(prefix: string): Promise<string[]> {
    try {
      return await this.primary.list(prefix);
    } catch {
  ...
  async write(input: StorageWriteInput): Promise<void> {
    try {
      await this.primary.write(input);
    } catch {
      await this.fallback.write(input);
    }
  }
  ...
  async delete(key: string): Promise<boolean> {
    ...
    try {
      deleted = (await this.primary.delete(key)) || deleted;
    } catch {
      // Continue to fallback.
    }

    try {
      deleted = (await this.fallback.delete(key)) || deleted;
    } catch {
      // Keep current status.
    }
...
function createStorageService(): StorageService {
  ...
  if (env.S3_ACCESS_KEY_ID && env.S3_SECRET_ACCESS_KEY && env.S3_BUCKET) {
    return new ResilientStorageService(new S3StorageService(), localStorage);
  }
