...
async function checkPrinter() {
  try {
    ...
    const data = result as { mode?: string; ok?: boolean };

    if (data.mode === "local" && data.ok === false) {
      return {
        status: "unhealthy",
        ...result,
        error: "No local browser executable found for printer fallback.",
      };
    }
