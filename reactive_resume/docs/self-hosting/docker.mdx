...
description: "A comprehensive guide to self-host Harbor Resume with Docker (Postgres + Printer), including a detailed environment variable reference and troubleshooting tips."
---

## Overview

Harbor Resume can be self-hosted using Docker in a matter of minutes, and this guide will walk you through the process. Here are some of the services you'll need to get started:
...
# to let the browser navigate to a non-public instance of Harbor Resume
...
SMTP_USER=""
SMTP_PASS=""
SMTP_FROM="Harbor Resume <noreply@harbor.dev>"
SMTP_SECURE="false"
...
```
  ...
  <Step title="Create compose.yml">
    This setup runs Postgres + Printer + Harbor Resume on a private Docker network.
...
```
    ...
    Harbor Resume should now be available at your `APP_URL` (for the example above: `http://localhost:3000`).
...
<AccordionGroup>
  ...
  <Accordion title="Printer">
    - **`PRINTER_ENDPOINT`**: Base URL where Harbor Resume reaches the printer service. In Compose: `http://printer:3000`. If using browserless with token authentication, include the token as a query parameter: `ws://printer:3000?token=your-secret-token`.
...
```
  ...
  <Accordion title="Email (SMTP, optional)">
    ...
    - **`SMTP_FROM`**: Default from address (for example, `Harbor Resume <noreply@harbor.dev>`).
...
To update your Harbor Resume installation to the latest available version, follow these steps:
...
Regular backups are essential to protect your data. Harbor Resume stores data in two places: the PostgreSQL database and file uploads (either local storage or S3).
...
Harbor Resume exposes a health check endpoint at `/api/health` that verifies the application and its dependencies are functioning correctly. If any critical service (such as the database connection) fails, the health check will return an unhealthy status.
...
This is particularly useful in high-availability setups where you have multiple instances of Harbor Resume. If one instance becomes unhealthy (for example, it loses its database connection), the reverse proxy will stop routing traffic to it until it recovers.
...
You can manually verify the health of your Harbor Resume instance:
...
<AccordionGroup>
  <Accordion title="The app container exits immediately">
    ...
    - **What to do**:  
      When the app container exits right away, you'll want to check the logs for more information about the error. Run the following command to view real-time logs from the Harbor Resume container:
      ```bash
      docker compose logs -f reactive-resume
      ```
  </Accordion>
...
<Accordion title="PDF export fails / printing is stuck">
  - **Common cause**: Harbor Resume can't reach the printer or the printer can't reach your app. - **Checks**: -
