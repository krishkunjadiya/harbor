...
This example uses [Traefik](https://traefik.io/) as a reverse proxy with automatic SSL certificate management via Let's Encrypt. Only the Harbor Resume app is exposed through Traefik—Postgres and the printer remain on an internal network.
...
services:
  ...
  reactive_resume:
    ...
    deploy:
      ...
      labels:
        - "traefik.enable=true"
        - "traefik.http.routers.app.rule=Host(`harbor.dev`)"
