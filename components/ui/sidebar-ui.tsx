...
function SidebarProvider({
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    () => ({
      ...
      org,
      universityRole }),
    [state, open, setOpen, isMobile, openMobile, setOpenMobile, toggleSidebar, portal, org, universityRole],
  )
...
function SidebarRail({
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border sm:flex -translate-x-1/2",
...
function SidebarContent({
  ...
  return (
    <ScrollArea
      ...
      className={cn(
        "flex flex-1 flex-col group-data-[collapsible=icon]:overflow-hidden min-h-0",
        className,
      )}
      {...props}
    />
